...
We demonstrate that two targeted changes to TurboQuant's value quantization—halving the group size and adopting true 3-bit packing—raise per-token cosine fidelity from 0.932 to 0.986 while preserving or improving compression ratios.
...
\item \textbf{True 3-bit packing} (8 values per 3 bytes, cosine 0.986 vs 0.932 at the same 64 B/token budget).
\end{enumerate}
...
\item \textbf{True 3-bit packing (C2)}: Design and implementation of a lossless 8-values-per-3-bytes packing scheme. Cosine similarity 0.986, approaching 4-bit quality (0.997) at near-2-bit storage.
...
\]
The QJL term decodes the residual $\bm{r} = \bm{k} - \hat{\bm{k}}_\text{MSE}$ as $\hat{\bm{r}} = (\alpha/d) \cdot \|\bm{r}\| \cdot S^\top \mathrm{sign}(S\bm{r})$, where $S \in \R^{d \times d}$ has iid $\mathcal{N}(0,1)$ entries and the scale $\alpha = \sqrt{\pi/2}$ is the unbiased coefficient $1/\Eop[|z|]$ for $z \sim \mathcal{N}(0,1)$. Under iid-Gaussian residuals this satisfies $\Eop[\hat{s}] = \langle \bm{q}, \bm{k} \rangle$—provably unbiased. The Gaussian assumption is an engineering choice baked into the upstream codec; \S\ref{sec:discussion} discusses when it breaks and how this branch exposes $\alpha$ as a calibratable knob.
...
The improvement in quality at equal bytes-per-token is the core result: 3-bit $G=32$ at 64 B/token achieves cosine similarity 0.986 vs 0.952 for 2-bit $G=16$—a $+0.034$ improvement at no storage cost increase.
...
\midrule
3 & 32 & \textbf{0.986} & 64 & \textbf{New sweet spot} (same budget as 2-bit gs=16) \\
...
\textbf{Conclusion} (three-time rule applied): Group size is the dominant free lever in value quantization. At 64 B/token, the 3-bit gs=32 configuration ($\Delta = +0.034$ vs 2-bit gs=16) is Pareto-superior to any 2-bit configuration at the same budget.
...
\addlegendentry{FP16}
% Annotation
\node[font=\tiny, red, above right, align=left] at (64, 0.986) {3-bit gs=32\\\textbf{+0.034}};
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Left}: Quality-compression Pareto front. At 64 B/token, 3-bit gs=32 (red, CosSim=0.986) is $+0.034$ above 2-bit gs=16 (blue, CosSim=0.952) at equal storage. The old default (2-bit gs=32, 48 B/tok) is Pareto-dominated by 3-bit. \textbf{Right}: Triton attention scoring speedup vs PyTorch at increasing context length. Triton approaches constant-time behavior (sub-linear scaling) due to compressed key reads; 60$\times$ speedup at $N=16{,}384$.}
...
(ii) \textit{Quality tier}: use \texttt{value\_bits = 3, group\_size = 32} — same 64 B/token as 2-bit gs=16, but $+0.034$ additional cosine (0.986 total).
...
\textbf{Scope: per-token adaptive precision on GQA.}
Our contributions concern \emph{uniform} low-bit value quantization, where the codec is strong. They do \emph{not} extend to pairing the rotated vector codec with a per-token adaptive-precision controller on grouped-query-attention (GQA) models, and this is the largest known scope boundary. We integrated TurboQuant+ with a \emph{Don't Waste Bits!}-style per-token importance controller (a learned router assigning bit-width $\in\{2,4,8,16\}$ per token) and evaluated \emph{task accuracy} (HellaSwag, length-normalized \texttt{acc\_norm}) rather than reconstruction cosine. On an MHA model (SmolLM-360M) the codec~$+$~router recipe works (42.0\% vs 42.6\% FP16). On a GQA model (Qwen2-0.5B: 2 KV heads shared 7:1 across 14 query heads) it fails to recover task accuracy: routing through the codec scores $0.36$ and even an \emph{oracle} that keeps 50\% of tokens losslessly stays at $0.348$ — indistinguishable from the uniform codec ($0.352$) and far below FP16 ($0.420$). Holding the routing, the protected set, and the model fixed, and swapping only the compressor for the other half from the rotated codec to scalar INT recovers the result to $0.412$, and a learned controller over scalar tiers reaches $0.416$ at 7.6 bits/token. The codec, not GQA or the router, is the limiter. The mechanism is the rotation: the RHT/WHT that spreads quantization error \emph{uniformly} across the head dimension — exactly what makes it a good uniform codec — \emph{delocalizes} each token's error across all $d$ coordinates, so per-token protection cannot localize and remove it, while GQA's 7:1 sharing amplifies the residual across query heads (MHA contains it to one head). The practical consequence: cosine similarity, our headline metric, stays at 0.986 while task accuracy collapses under per-token routing on GQA, so adaptive-precision claims must be validated on task accuracy, not cosine or perplexity alone. For adaptive-precision KV compression on GQA the working recipe is a learned controller over \textbf{scalar-INT tiers $\{4,8,16\}$ (drop the 2-bit tier)}; whether a localized/blockwise rotation can keep the codec's low-bit edge \emph{and} per-token protectability on GQA is open for a future codec revision.
...
(ii) \textit{Cosine is a proxy, not task accuracy}: the headline quality numbers are per-token reconstruction cosine, which we show above does not predict task accuracy under per-token adaptive precision on GQA. Full perplexity/MMLU/HumanEval on Wikitext-103/C4 with patched TurboQuant via vLLM is outside our current compute budget.
...
(iv) \textit{GQA $+$ per-token adaptive precision}: as detailed above, the rotated vector codec does not recover task accuracy under a per-token importance controller on GQA; a scalar-INT tiering does. The negative result is single-model (Qwen2-0.5B) and single-benchmark (HellaSwag, $n{=}250$, CI $\approx\pm0.057$) at a quantization-fragile 0.5B scale; the codec-vs-scalar dissociation ($0.348$ vs $0.412$ at identical routing) is the robust signal rather than any single cell.

\textbf{Per-layer QJL scale calibration.}
The QJL decoder constant $\alpha = \sqrt{\pi/2}$ is the unbiased coefficient under iid-Gaussian residuals (\S\ref{sec:turboquant}). Any per-layer centroid retune shifts the residual distribution and miscalibrates this default. We expose $\alpha$ as a constructor knob on \texttt{TurboQuantProd} / \texttt{TurboQuantKVCache} and provide a closed-form MSE-min calibrator in \texttt{turboquant.codebook.calibrate\_qjl\_scale}. Fitting the codec's decoder $\hat{\bm{r}} = (\alpha/d) \|\bm{r}\| \bm{g}$ with $\bm{g} = S^\top \mathrm{sign}(S\bm{r})$ over a set of captured residuals admits
\[
\alpha^*_{\mathrm{MSE}} = d \cdot \frac{\sum_i \|\bm{r}_i\| \langle \bm{r}_i, \bm{g}_i \rangle}{\sum_i \|\bm{r}_i\|^2 \|\bm{g}_i\|^2},
\]
which at finite $d=64$ for iid Gaussians evaluates to $\sqrt{2/\pi} / (1 + 2/\pi) \approx 0.4875$—distinct from the unbiased $\sqrt{\pi/2} \approx 1.2533$. The MSE-min vs unbiased gap shrinks as $d \to \infty$. Cosine-similarity-optimal $\alpha$ has no closed form (rational in $\alpha$ per sample) and requires a 1D search; we leave the empirical comparison of MSE-min vs cosine-max $\alpha$ on real captures to future work. With this knob, per-layer codebook retunes can ship as a coupled $(\text{centroids}, \text{boundaries}, \alpha)$ artefact without disturbing the default code path.
...
We demonstrated that TurboQuant's 2-bit value quantization quality gap (CosSim 0.932) can be closed via two targeted changes: halving the group size from 32 to 16 (+0.020 cosine, free) and adopting true 3-bit packing (+0.055 cosine vs 2-bit default, 33\% more bytes). At equal 64 B/token storage, 3-bit values (CosSim 0.986) are strictly superior to any 2-bit configuration. We validated all three Triton fused decode kernels on a real GPU (60$\times$ speedup at 16k context) and proved the Randomized Hadamard Transform is quality-identical to QR rotation with 102$\times$ less memory. Together, these improvements enable 2.39 million token inference on a commodity RTX A4000—making million-token contexts practically viable without specialized hardware. These gains are for \emph{uniform} low-bit quantization; pairing the rotated codec with a per-token adaptive-precision controller on grouped-query-attention models is a separate regime where the rotation defeats per-token protection and a scalar-INT tiering is required instead (\S\ref{sec:discussion}).
